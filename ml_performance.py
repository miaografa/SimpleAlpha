...
class FastMLFactorEvaluator:
    '''减少了数据使用，便于快速测试因子效果'''
    def __init__(self, method, record_names:list, factor_names:list, params_dict={}):
        ...
        if len(params_dict) == 0:
            self.params_dict = {'n_jobs': -1}
        else:
            self.params_dict = params_dict

    def evaluate(self, dataloader:DataLoader, y_col, subset_data=0.1):
        '''
        评估所有的因子
        :param dataloader: DataLoader
        :param y_col: y的列名
        :param subset_data: 是否只评估部分数据，如果是，那么按照subset比例随机选取数据

        '''
        valid_year_month = dataloader.get_valid_year_month()
        subset_num = int(len(valid_year_month) * subset_data)
        ind = np.random.choice(range(len(valid_year_month)), subset_num, replace=False)
        self.valid_year_month = [valid_year_month[i] for i in ind]
        # 划分数据集
        train_length = int(0.7 * len(self.valid_year_month))
        train_ind = np.arange(train_length)
        test_ind = np.arange(train_length, len(self.valid_year_month))
        # 获取数据
        ...
        # 记录结果
        self.record_df['record_names'] = self.record_names
        self.record_df['f1'] = f1_list
        self.record_df['auc'] = auc_list
...
class MLFactorEvaluator:
    def __init__(self, method, record_names:list, factor_names:list, cv=5, params_dict={}):
        assert len(record_names) == len(factor_names), 'record_names和factor_names长度不一致'
        self.record_names = record_names
        self.factor_dict = dict(zip(record_names, factor_names))
    ...
    def evaluate(self, dataloader:DataLoader, y_col, subset_data=1):
        '''
        评估所有的因子
        :param dataloader: DataLoader
        :param y_col: y的列名
        ...
        '''
        valid_year_month = dataloader.get_valid_year_month()  # 有效的年月列表
        if subset_data < 1:
            # 随机选取部分数据
            subset_num = int(len(valid_year_month) * subset_data)
            ind = np.random.choice(range(len(valid_year_month)), subset_num, replace=False)
            ...
            self.valid_year_month = valid_year_month
    ...
    def time_series_split(self, time_length:int) -> list:
        ...
        :return:
            [(train_ind, test_ind), ...]
        '''
        tscv = TimeSeriesSplit(n_splits=self.cv, max_train_size=24, test_size=3)
